package ps.demo.lqbs.repository;
...
public interface FileRepository extends JpaRepository<FileRecord, Long> {
    ...
    @Modifying
    @Transactional
    @Query("update ps.demo.lqbs.entity.FileRecord f set f.status = :status, f.updatedAt = :updatedAt where f.id = :fileId and f.status = :oldStatus")
