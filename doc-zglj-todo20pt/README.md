...
### There is a simple CRUD example using JPA: doc-zglj-todo20pt
- Solution 1:
...
URLs:
- http://localhost:18961/api-docs
- http://localhost:18961/swagger-ui/index.html
- http://localhost:18961/api/books/
- http://localhost:18961/actuator
- http://localhost:18961/upload2.html
H2 DataBase:
- http://localhost:18961/h2-console
<pre>
    Driver Class: org.h2.Driver
    JDBC URL:     jdbc:h2:mem:doc-zglj-todo20pt;MODE=MYSQL;DB_CLOSE_DELAY=-1
    User Name:    sa
</pre>
...
curl -ikLv --request GET \
  --url http://localhost:18961/api-docs \
  --header 'User-Agent: insomnia/10.1.0'

curl -ikLv --request GET \
  --url http://localhost:18961/api/books/ \
  --header 'User-Agent: insomnia/10.1.0'

curl -ikLv --request GET \
  --url http://localhost:18961/actuator \
  --header 'User-Agent: insomnia/10.1.0'
...
- Make sure you mvn rebuild the latest jar!!!
- Please set NEXUS_USER & NEXUS_PASS if you'd like to connect to NEXUS.
- docker push xxx.com/path/repo/doc-zglj-todo20pt
- If no NEXUS repo, comment out "docker push xxx.com/path/repo/doc-zglj-todo20pt" in dockerBuildPushAndGenDeployYaml.sh
- If no NEXUS repo, fix the: "xxx.com/path/repo/doc-zglj-todo20pt:THE_APP_VERSION" in deploy-template.yaml
...
- OR: kubectl -n nsapp logs -f $(kubectl -n nsapp get pods --field-selector status.phase=Running --no-headers -o custom-columns=":metadata.name" | grep doc-zglj-todo20pt | head -1)
- OR: kubectl -n nsapp -it exec $(kubectl -n nsapp get pods --field-selector status.phase=Running --no-headers -o custom-columns=":metadata.name" | grep doc-zglj-todo20pt | head -1) -- bash -c "ls"
- Check container: kubectl -n nsapp -it exec podxxx -- bash 
- OR: kubectl -n nsapp -it exec ngx-65f68876c8-ksc74  -c 1st -- /bin/sh
--- 
- Visit Application via:
  URLs:
- http://localhost:38961/api-docs
- http://localhost:38961/swagger-ui/index.html
- http://localhost:38961/api/books/
- http://localhost:38961/actuator
-
H2 DataBase:
- http://localhost:38961/h2-console
<pre>
    Driver Class: org.h2.Driver
    JDBC URL:     jdbc:h2:mem:doc-zglj-todo20pt;MODE=MYSQL
    User Name:    sa
</pre>
...
mvn clean test "-Dspring.profiles.active=dev" "-Dtest=ps.demo.zglj.cucumber.CucumberTestRunner" "-Djasypt.encryptor.password="
